...
@app.route('/api/create_journal', methods=['POST','GET'])
def create():
...
@app.route("/api/saved_journal", methods=['POST','GET'])
def saved_journal():
    if 'loggedin' not in session:
        return jsonify({'msg': 'You are not logged in', 'status': 'error'})
...
# Selecting the tasks for the plant's treatment based on the session of disease name
@app.route("/api/select_additional_task", methods=['POST','GET'])
def select_additional_tasks():
        if 'loggedin' not in session:
            return jsonify({'msg': 'You are not logged in', 'status': 'error'})
